...
const App: React.FC = () => {
  ...
  const handleSaveProject = async () => {
    if (!clientDetails.projectName) {
      setToast({ message: 'Please enter a Project Name before saving.', type: 'error' });
      return;
    }
    setIsSaving(true);
    try {
      ...
      if (projectId) {
        ...
        projectId = newProject.data?.id || null;
        setCurrentProjectId(projectId);
      }

      if (projectId) {
        // 2. Save Rooms (This is a simplified overwrite logic for demo)
        // Ideally, you'd track individual room IDs. For now, we clear old rooms for this project and re-add.
        const existingRooms = await client.models.Room.list({
            filter: { projectId: { eq: projectId }}
        });
        
        // Delete existing cloud rooms to prevent dupes/orphans in this simple implementation
        await Promise.all(existingRooms.data.map(r => client.models.Room.delete({ id: r.id })));

        // Create new rooms
        await Promise.all(rooms.map(room => client.models.Room.create({
            projectId: projectId!,
            name: room.name,
            answers: JSON.stringify(room.answers),
            boq: JSON.stringify(room.boq)
        })));
  ...
  const handleLoadProjectSelected = async (projectId: string) => {
    try {
      const project = await client.models.Project.get({ id: projectId });
      const roomList = await client.models.Room.list({ filter: { projectId: { eq: projectId } } });

      if (project.data) {
        const details = JSON.parse(project.data.clientDetails as string);
        setClientDetails(details);
        setCurrentProjectId(projectId);

        const loadedRooms: Room[] = roomList.data.map(r => ({
            id: r.id, // Use cloud ID
            name: r.name || 'Untitled Room',
            answers: JSON.parse(r.answers as string || '{}'),
            boq: JSON.parse(r.boq as string || 'null'),
            isLoading: false,
            error: null,
            isValidating: false,
            validationResult: null
        ...
        setActiveRoomId(loadedRooms.length > 0 ? loadedRooms[0].id : null);
      ...
      <div className={`min-h-screen bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-200 transition-colors duration-300`}>
        ...
        <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          <div className="flex justify-end mb-4 gap-2 no-print">
             <div className="text-sm text-slate-500 flex items-center mr-4">
            ...
            <aside className="md:col-span-1 space-y-6">
              <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700 space-y-3 no-print">
                ...
                <button onClick={handleSaveProject} disabled={isSaving} className="w-full inline-flex items-center justify-center px-4 py-2 border border-slate-300 dark:border-slate-600 text-sm font-medium rounded-md shadow-sm text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600 disabled:opacity-50">
                  {isSaving ? <LoaderIcon /> : <SaveIcon />} {isSaving ? 'Saving...' : 'Save Project'}
                </button>
                <button onClick={() => setIsLoadModalOpen(true)} className="w-full inline-flex items-center justify-center px-4 py-2 border border-slate-300 dark:border-slate-600 text-sm font-medium rounded-md shadow-sm text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600">
        ...
        <ProjectLoadModal
          ...
          onLoadProject={handleLoadProjectSelected}
        />
