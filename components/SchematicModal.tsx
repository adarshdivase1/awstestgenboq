...
const SchematicModal: React.FC<SchematicModalProps> = ({ isOpen, onClose, isLoading, error, imageUrl, onRegenerate, onDelete }) => {
  if (!isOpen) {
    return null;
  }

  const handleDelete = () => {
    if (window.confirm('Are you sure you want to delete this schematic? You will need to generate it again.')) {
      onDelete();
      onClose();
    }
  };
    ...
    >
      ...
      >
        <div className="flex justify-between items-center mb-4 flex-shrink-0">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">Room Schematic Diagram</h2>
        ...
        <div className="flex-grow flex items-center justify-center overflow-hidden bg-slate-100 dark:bg-slate-900/50 rounded-md">
          {isLoading && (
            <div className="text-center text-slate-500 dark:text-slate-400">
              ...
              <p className="mt-4 text-lg">Generating detailed schematic...</p>
              <p className="text-sm text-slate-400 dark:text-slate-500">This is a complex task and may take a minute. Please be patient.</p>
            </div>
          )}
          {error && !isLoading && (
            <div className="text-center text-red-600 dark:text-red-400 p-8">
              <h3 className="text-lg font-semibold">Schematic Generation Failed</h3>
              <p>{error}</p>
            </div>
          )}
          {imageUrl && !isLoading && (
            <img 
              src={imageUrl} 
              alt="Generated room schematic diagram" 
